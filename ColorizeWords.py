...
class ColorizeWordsCommand(sublime_plugin.WindowCommand):
    def get_words(self, text):
        if USE_REGEX:
            return list(
                filter(lambda x: x and x != " ", re.split(r"((?:\\ |[^ ])+)", text))
            )
        else:
            return text.split()
    ...
    def run(self):
        ...
        if not view:
            return
        word_list = self.get_words(view.settings().get("colorize_text", ""))
        ...
        v = self.window.show_input_panel(
            prompt, display_list, None, self.on_change, self.on_cancel
        )
        sel = v.sel()
        sel.clear()
    ...
    def on_change(self, text):
        stamp = time.time()
        self.stamp = stamp
        sublime.set_timeout(lambda: self.colorize(text, stamp), 500)

    def colorize(self, text, stamp):
        if self.stamp != stamp:
            return
        self.window.run_command("uncolorize_words")
        view = self.window.active_view()
        ...
        for word in words:
            ...
            view.add_regions(
                "colorize_word_%d" % size,
                regions,
                SCOPES[size % len(SCOPES)],
                "",
                sublime.HIDE_ON_MINIMAP,
            )
            size += 1
        view.settings().set("colorize_size", size)
        view.settings().set("colorize_text", text)

    def on_cancel(self):
        self.window.run_command("uncolorize_words")
        view = self.window.active_view()
        view.settings().erase("colorize_text")


class UncolorizeWordsCommand(sublime_plugin.WindowCommand):
    def run(self):
        view = self.window.active_view()
        if not view:
            return
        size = view.settings().get("colorize_size", 0)
        for i in range(size):
            view.erase_regions("colorize_word_%d" % i)
        view.settings().set("colorize_size", 0)


class ColorizeSettingsCommand(sublime_plugin.WindowCommand):
    def run(self):
        names = [
    ...
    def on_done(self, selected):
        if selected == -1:
            return
        settings = sublime.load_settings("ColorizeWords.sublime-settings")
        if selected == 0:
        ...
        sublime.save_settings("ColorizeWords.sublime-settings")


class ColorizeKeywordsCommand(sublime_plugin.EventListener):
    def handleTimeout(self, view, stamp):
        if self.stamp != stamp:
            return
        self.colorizeKws(view)
...
def get_settings():
    ...
    setting = sublime.load_settings("ColorizeWords.sublime-settings")
    ...
    KEYWORD_MAP = setting.get("permanent_colorize_keyword_color_mappings", [])
    return setting
